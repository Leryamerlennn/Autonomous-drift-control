...
private:
    float x[2] = {0, 0};  
    float P[2][2] = {{1, 0}, {0, 1}};  
    float Q[2] = {0.001, 0.001};  // Noise of process
    float R = 0.1;              // Noise of measure
...
bool isHardDrift(float gyroZ, float accelY) {
    return (abs(gyroZ) > 200) || (abs(accelY) > 3.0); // Approximate measures for hard driving
}
...
void loop()
{
  ...
  if (currentMillis - lastPrintMillis > INTERVAL_MS_PRINT) {
    ...
    Serial.print(kalmanAngles.x, 3);
    Serial.print(", ");
    Serial.print(kalmanAngles.y, 3);
    Serial.print(", ");
  ...
  if (isHardDrift(normalized.gyroscope.z, normalized.accelerometer.y)) {
      updateAlpha(0.99); // More trust to gyroscope measure, rather than accel
  } else {
      updateAlpha(0.9);
  }
}
